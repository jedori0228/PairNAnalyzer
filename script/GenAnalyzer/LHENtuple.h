...
// Wed Mar  7 17:49:32 2018 by ROOT version 6.08/00
// from TTree LHEF/Analysis tree
// found on file: PairProduction_MuMu_ZR2000_N100_WR5000_NLO.root
//////////////////////////////////////////////////////////
...
public :
   ...
   //==== For Pair
   GenParticle N[2], Lepton[2], Jet1[2], Jet2[2];
...
#ifdef LHENtuple_cxx
LHENtuple::LHENtuple(TString filename) : fChain(0) 
{
  TFile *f = new TFile(filename);
  fChain = (TTree *)f->Get("LHEF");
  Init();
}
...
Int_t LHENtuple::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t LHENtuple::LoadTree(Long64_t entry)
{
...
void LHENtuple::Show(Long64_t entry)
{
...
Int_t LHENtuple::Cut(Long64_t entry)
{
...
TH1* LHENtuple::GetHist(TString hname){
...
void LHENtuple::FillHist(TString histname, double value, double weight, int n_bin, double x_min, double x_max){

  if(GetHist(histname)) GetHist(histname)->Fill(value, weight);

  else{
    maphist[histname] = new TH1D(histname, "", n_bin, x_min, x_max);
    maphist[histname]->Fill(value, weight);
  }
